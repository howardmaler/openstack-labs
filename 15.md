...
#### 0. Core Setup For all nodes (Item 01 though 06 manually initialized with the following 5 steps
...
1. Install ubuntu 16.04
   ...
   ###### NOTE: _Placing "-y" at the end of an apt-get instruction is the same as selecting y at a yes/no prompt and is optional as it only allows for unattended execution_
...
2. Add Default User(s) and Add to Sudoers List
   ...
   ####### NOTE: You will be prompted to provide a password, use the same password from step 1.3, but you will only need it for a short period of time. Again it can easily changed later ...
   ...
   ####### NOTE: NOPASSWD at that position will allow that user to sudo without the need to enter a password each time which can get annoying.
   ...
   ####### NOTE: Repeat these 4 steps to add more users if necessary


2. Configure the network and access

   ####### NOTE  Each node system will require a static IP address.

   1. Open /etc/network/interfaces in vim

      `ubuntu@controller:~ # sudo vim /etc/network/interfaces`

   2. Locate the line containing _iface [device_name] inet **dhcp**_, and replace **_dhcp_** with **_static_** and add the following block. It will look something like the following 2 lines. The following example uses a network interface name of _**en33**_, this name will vary from system to system. Be sure to use that name and do not modify. If you do that will cause the network adaptor not to function correctly.

       ```
       > auto en33              <---- WARNING, en33 will vary from system to system. DO NOT MODIFY the name itself, google "predictable network interface names"
       > iface en33 inet dhcp   <---- COMMENT THIS LINE OUT
       ```

   3) Modify that to Add the following keeing in mind that **ens160** should reflect the proper device name.

       ```
       auto ens160

       # iface en33 inet auto
       iface en33 inet static            <---- Notice that _auto_ is replaced with _static_, that is the only change for this line

       address 10.3.0.11                 <---- Need to specify the rest of these based on address this is only an example
       netmask 255.255.255.0             <---- Pretty standard again this is only an example
       gateway 10.3.0.1                  <---- Need a gateway to external sources
...
|Contents|Environment|Keystone|Glance|Nova|Neutron|Horizon|Cinder|Swift|Heat|Ceilometer|Test|
|---|---|---|---|---|---|---|---|---|---|---|---|
