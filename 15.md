...
1. Install ubuntu 16.04
    ...
    1) You can name this system anything, but it is strongly recommended that you name it according to the node.
        ...
        Examples
            ...
            - *controller:*  the system where the _controller node_ is housed
            - *compute1:*    the system where the _first compute node_ is housed
            - *neutron:*     the system where the _neutron node_ is housed
            - *compute[X]:*   where [X] represents a number is the system where the _[X] compute node is housed'
...
2. Configure the network
    ...
    1) Modify the ip address to use a static ip rather than one provided from the DHCP server



 Each node will require ystem will require a static IP address.

    1) Open the /etc/network/interfaces with vim.

    **
    ``
    ubuntu@ansible:~ # sudo vim /etc/network/interfaces
    ``
    **

    2) Locate the following 2 lines, and comment out the line continain **dhcp** _they should be together but this is not necessarily the case_

        ```
        > auto ens160              <---- WARNING, this name (ens160) will vary, google "predictable network interface names"
        > iface ens160 inet dhcp   <---- COMMENT THIS LINE OUT
        ```
    ...
    3) Add the following keeing in mind that **ens160** should reflect the proper device name.
