...
2. Configure the network and access
    ...
    ```
    auto en33
    iface en33 inet dhcp
    ```


    DO NOT MODIFY the name itself, google "predictable network interface names"

    <---- WARNING, en33 will vary from system to system.
    ...
    3) Modify that to Add the following keeing in mind that **ens160** should reflect the proper device name.
        ...
        iface en33 inet static            <---- Notice that _auto_ is replaced with _static_
        address 10.3.0.11                 <---- Remember these are only sample values
        netmask 255.255.255.0
        gateway 10.3.0.1
        dns-nameservers 192.168.201.80
        ```
