...
## Configuration Walkthrough To Create the Mitaka Lab Platform ####
...
#####02. Creating the Controller Environment for Mitaka
...
1. Security
    ...
    i. **Credential Management**, Openstack has many values for services and databases which will be required throughout this walkthorough. Most are listed in the table below. The value has been defaulted to openstacklogin. Some may require only a password, some may require a user id, some may also require the host ip or hostname.
    ...
    ###### NOTE: _In this OpenStack setup walkthrough SERVICE_PASS refers to a service account password and SERVICE_DBPASS refers to a database password. These are both reflected in the table above as suffixes that are prepended by the name of the service or database for which the login information is relevant ..._

    ii. In addition to the standard Openstack credentials there are some additional values which also will be required during the walkthrough, those are laid out in the follwoing table.
    ...
    |08|OS_URL|http://controller:35357/v3|
    |09|region|RegionOne|
    |10|default-domain|Default Domain|
    |11|and-so-on|and-so-forth|
    |12|ADMIN_TOKEN| {{Generated Hash}}|

    iii. Generate a hash key for later use into a file in the user home directory called _.openstack-dat_
    open that file in vim. The generated hash will appear at the top of the file, add each of the two tables
    above replacing {{Generated Hash}} with the value of the hash.
