...
1. Security
    ...
    i. Generate a hash key for later use into a file in the user home directory called .haskkey

    ```
    openssl rand -hex 10 > .hashkey
    ```

    ii. **Credential Management**, the following table provides a list of services and databases, some of these require only a password some also require a user-id keep in mind that some user-ids also include the host name or host address. Open a new file for editing in the user home directory called .openstack_data and populate it from the following table.

    |Database or Node Name|Description|User Id|Password|
    |---|---|---|---|
    ...
    In the OpenStack step by step SERVICE_PASS refers to a service account password and SERVICE_DBPASS refers to a database password. These two suffixes are prepended by the name of a node for that password.
    iii. Create a common config table like this to centralize variable configuration
